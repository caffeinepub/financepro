 ...
 * @returns The parameter value if found, null otherwise (never returns empty string for missing params)
 */
 ...
 * @returns The stored value if found, null otherwise (never returns empty string)
 */
export function getSessionParameter(key: string): string | null {
    try {
        const value = sessionStorage.getItem(key);
        // Ensure we return null for missing values, not empty string
 ...
 * @returns The parameter value if found, null otherwise (never returns empty string)
 */
export function getPersistedUrlParameter(paramName: string, storageKey?: string): string | null {
    ...
    if (urlValue !== null && urlValue !== '') {
        // Store in session for persistence (only if non-empty)
 ...
 * @returns The secret value if found (from hash or session), null otherwise (never empty string)
 */
export function getSecretFromHash(paramName: string): string | null {
    ...
    // Only store and return if secret is non-null and non-empty
 ...
 *
 * @param paramName - The name of the secret parameter
 * @returns The secret value if found, null otherwise (never returns empty string)
 */
